...
-Implement initialization and regularization
"""
...
class Network(object):
    ...
    def __SGD(self,X,y_pred,y_true):
        if self.loss == 'mse':
    ...
    #defining backprop
    def __backward_prop(self,inputs,weights,y_true):
        updated_weights = []
        if self.optimization == 'SGD':
            y_pred,sample_y_true,_ = self.__batch_predict(inputs, y_true, 1)
            for i in reversed(range(len(weights))):
                delta = self.__SGD(inputs[i], y_pred, sample_y_true)
                updated_weights.append((weights[i] - delta*np.ones(weights[i].shape)))     
        return list(reversed(updated_weights))
            
    #fit data to model by training    
    def fit(self,X_in,y_in,
            epochs=100,batch_size=50,seed=42):
        ...
        for epoch in range(epochs):
            if batch_size is None:
                ...
                sample_y_pred,sample_y_true,sample_activities = self.__batch_predict(X_in, y_in,batch_size)
                self.final_weights = self.__backward_prop(X_in,self.final_weights,y_in)
                outputs = sample_y_pred
            
                cur_loss = self.__mse(outputs,sample_y_true) #loss for given epoch
                if epoch % 10 == 0:
    ...
    def __batch_predict(self,X_in,y_in,batch_size):
        
        batches = self.__shuffle_batch(X_in,y_in,batch_size)
...
model = Network(n_inputs, n_outputs,learning_rate=0.1)
...
print(f"{model.accuracy(y_pred, y_test) * 100}%")
